...
class Detail(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["tags"] = tags
        context["re_posts"] = Question.objects.all().order_by('?')[:15]
...
class RootPage(View):
    ...
    context  =  {
            "top_btc": Question.objects.order_by('-answercount')[:20],
            "top_eos": eosqn.objects.order_by('-answercount')[:20],
            "top_eth": ethqn.objects.order_by('-answercount')[:20],
            "top_news": Post.objects.order_by('-updated')[:5],
                }
